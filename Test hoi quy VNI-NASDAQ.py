...
def plot_2_figures(df):
	columns = df.columns
	plt.plot(df.index, df[columns[0]] / 10, label = 'NASDAQ', color = 'red')
...
df = pd.DataFrame(columns = ['Date', 'NASDAQ', 'VNI'])
#Read DJI
df1 = pd.read_csv('^IXIC.csv')
#Read VNIndex
...
for each_day in list(df.index):
	if each_day in set(df1.index) and each_day in set(df2.index):
		df['NASDAQ'][each_day] = df1['Close'][each_day]
...
remove = df[pd.isnull(df['NASDAQ'])][pd.isnull(df['VNI'])].index
#Replace nan with previous value
for i in range(len(df)):
	if math.isnan(df['NASDAQ'][i]):
		df['NASDAQ'][i] = df['NASDAQ'][i - 1]
	if math.isnan(df['VNI'][i]):
		df['VNI'][i] = df['VNI'][i - 1]

#Now run regression
regr = linear_model.LinearRegression()
regr.fit(np.array(df.NASDAQ).reshape(-1, 1), np.array(df.VNI).reshape(-1, 1))
