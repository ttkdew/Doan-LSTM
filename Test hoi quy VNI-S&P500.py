...
def plot_2_figures(df):
	columns = df.columns
	plt.plot(df.index, df[columns[0]] , label = 'SP500', color = 'red')
...
df = pd.DataFrame(columns = ['Date', 'SP500', 'VNI'])
#Read DJI
...
for each_day in list(df.index):
	if each_day in set(df1.index) and each_day in set(df2.index):
		df['SP500'][each_day] = df1['Close'][each_day]
...
remove = df[pd.isnull(df['SP500'])][pd.isnull(df['VNI'])].index
#Replace nan with previous value
for i in range(len(df)):
	if math.isnan(df['SP500'][i]):
		df['SP500'][i] = df['SP500'][i - 1]
	if math.isnan(df['VNI'][i]):
		df['VNI'][i] = df['VNI'][i - 1]

#Now run regression
regr = linear_model.LinearRegression()
regr.fit(np.array(df.SP500).reshape(-1, 1), np.array(df.VNI).reshape(-1, 1))
